...
function render(filter = "all") {
  grid.innerHTML = plays
    ...
    .map(play => `<article class="play ${play.phase}" id="play-${String(play.number).padStart(2,"0")}" data-content-id="${play.id}">
      ...
      <div class="starter" data-content-id="${play.tool.id}"><small>Free Starter Tool</small><strong><a href="${playUrl(play)}#starter-tool">${play.tool.title}</a></strong><a class="tool-link" href="${playUrl(play)}#starter-tool">About this tool →</a></div>
    </article>`).join("");
}
