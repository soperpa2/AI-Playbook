/**
 * Essentials member dashboard and browser-persistence adapter.
...
function readEssentialsJson(key, fallback) {
...
function renderEssentialsAccount() {
  const member = foundationMember();
  const learningAssessment = readEssentialsJson(FOUNDATION_LEARNING_ASSESSMENT_KEY, null);
  const learningProgress = readEssentialsJson(FOUNDATION_LEARNING_PROGRESS_KEY, {});
  ...
  accountMain.innerHTML = `
    <section class="page-hero panel"><p class="eyebrow">Essentials Edition</p><h1>My Account</h1><p>A simple personal account for free members. Join to download Essentials Tools, keep work in this browser, and become eligible to review new tools before release.</p></section>
