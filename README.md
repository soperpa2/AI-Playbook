...
- Planning and implementation framework:
  ...
  - DEPLOY: Plays 10-11
  - GOVERN: Plays 12-13
- Individual generated pages for each play.
...
The full platform is published from the repository root and the customer-facing Foundation Edition is maintained under the internal [`mvp/`](mvp/) directory. Both editions use the shared catalog in [`content/public-catalog.js`](content/public-catalog.js), the rules in [`content/publication-policy.json`](content/publication-policy.json), and canonical graphics from [`assets/`](assets/). Shared content and graphics are changed once at their canonical source so both editions remain aligned.
